...
export const storeVerifications = pgTable(
  "store_verifications",
  {
    ...
    // PRD § 7 + stores 테이블과 동일 enum (v0005 마이그레이션). 진행 중이면
    // 'pending', cron 자동 통과 'auto_approved', 매뉴얼 큐 'manual_review',
    // 운영자 거부 'rejected'.
    ...
    lastRevalidationAt: timestamp("last_revalidation_at", {
      withTimezone: true,
    }),
  ...
  (table) => [
    check(
      "store_verifications_verification_status_check",
