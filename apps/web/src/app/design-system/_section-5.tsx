...
// 1:1 reproduction of docs/design/handoff/app-2.jsx Section 5 (Components).
// 12 blocks: Button · Card · Input · Select · Datepicker · Modal · Sheet ·
// Toast · Badge · Avatar · Tabs · Navigation. Class names match components.css.
...
function ButtonBlock() {
  ...
  const ChevR = Icon.chevR;
  const Plus = Icon.plus;
  const More = Icon.more;
  const ArrowUR = Icon.arrowUR;
  return (
...
function FieldDemo({
  ...
  return (
    <div style={{ minWidth: 240 }}>
      <div className={cls} onClick={() => setF(true)}>
...
function SelectDemo({
  ...
  const initialMulti = multi ? [items[0], items[2]] : null;
  const [selMulti, setSelMulti] = useState<string[]>(
    (initialMulti as string[] | null) ? (initialMulti as string[]) : [],
  );
  const [selSingle, setSelSingle] = useState<string | null>(null);
          ...
          {selMulti.map((s) => (
            <span className="chip kr" key={s}>
              ...
              <span
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setSelMulti(selMulti.filter((x) => x !== s));
                }}
              >
                ✕
              </span>
          ...
          <div className="select-pop">
            {items.map((it) => (
              <div
                key={it}
                className={
                  "select-item" + (selMulti.includes(it) ? " selected" : "")
                }
                onClick={() =>
                  setSelMulti(
                    selMulti.includes(it)
                      ? selMulti.filter((x) => x !== it)
                      : [...selMulti, it],
                  )
                }
              >
                <span className="kr">{it}</span>
                {selMulti.includes(it) && <span className="check">✓</span>}
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div ref={ref} style={{ position: "relative", minWidth: 240 }}>
      ...
      >
        {selSingle ? (
          <span className="selected kr">{selSingle}</span>
        ) : (
      ...
      {open && (
        <div className="select-pop">
          {items.map((it) => (
            <div
              key={it}
              className={"select-item" + (selSingle === it ? " selected" : "")}
              onClick={() => {
                setSelSingle(it);
                setOpen(false);
              }}
            >
              <span className="kr">{it}</span>
              {selSingle === it && <span className="check">✓</span>}
            </div>
          ))}
        </div>
      )}
...
function DatepickerBlock() {
  ...
  const dows: Array<[string, string]> = [
    ["S", "일"],
    ["M", "월"],
    ["T", "화"],
    ["W", "수"],
...
function SheetBlock() {
  const ChevR = Icon.chevR;
  const X = Icon.x;
  const innerCellStyle: CSSProperties = {
    padding: "12px 14px",
              ...
              <div className="phone-bottom-sheet">
                ...
                <div style={{ marginTop: 16, display: "grid", gap: 8 }}>
                  <div style={innerCellStyle}>
                    ...
                    <ChevR size={16} color="var(--gray-500)" />
                  </div>
                  <div style={innerCellStyle}>
                    <div>
                      <div style={{ fontWeight: 600, fontSize: 13 }}>
...
function NavBlock() {
  ...
  const sideItems: Array<[keyof typeof Icon, string, boolean]> = [
  ...
  const collapsedKeys: Array<keyof typeof Icon> = [
    "layout",
    "inbox",
    "calendar",
    "scissors",
  ...
  const TabSearch = Icon.search;
  const TabCal = Icon.calendar;
  const TabMsg = Icon.message;
  const TabUser = Icon.user;
  return (
    <div className="component-block">
      <div className="component-title">
        ...
        <div>
          ...
          <div className="nav-header">
            ...
            <div className="search">
              <Search size={14} /> Search bookings, customers, services…
            </div>
