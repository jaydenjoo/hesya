  ...
  return (
    <CustomerFrame>
      ...
      <div className="px-6 pb-8 pt-4">
        {/* reference schedule.css L49-81: back btn 36px peach-100 + Fraunces italic 22/500 title + spacer */}
        <header className="mb-4 flex items-center gap-3">
          <Link
            href={`/c/store/${store.id}`}
            aria-label={tStoreDetail("title", { name: store.name })}
            className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-full bg-hesya-peach-100 text-hesya-navy-900 transition hover:bg-hesya-peach-200"
          >
            <span aria-hidden="true" className="text-[16px]">
              ←
            </span>
          </Link>
          <h1 className="font-heading text-[22px] font-medium italic leading-tight tracking-[-0.01em] text-hesya-navy-900">
            {t("heading")}
          </h1>
