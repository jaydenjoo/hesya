...
interface BuildInput {
  ...
  locale: Locale;
  storeName: string;
  /** manual_review_queued/auto_rejected_nts 시 사유 (선택) */
  reason?: string;
}
...
type Templates = Record<
  KycResultKind,
  { subject: (s: string) => string; body: (s: string, r?: string) => string }
>;
...
export function buildKycNotification(input: BuildInput): BuiltMessage {
  ...
  const map = MESSAGES[input.locale] ?? MESSAGES[DEFAULT_LOCALE] ?? MESSAGES.en;
  const tmpl = map[input.kind];
  return {
    subject: tmpl.subject(input.storeName),
    body: tmpl.body(input.storeName, input.reason),
  };
}
