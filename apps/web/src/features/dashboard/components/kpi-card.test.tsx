...
describe("KpiCard — Epic 4 ε / D4-D1 + M6.2 design refresh", () => {
  it("active state — peach-100 border + white bg + navy-900 value", () => {
    ...
    expect(card?.className).toContain("border-hesya-peach-100");
    expect(card?.className).toContain("bg-white");
    expect(container.querySelector(".text-hesya-navy-900")).toBeTruthy();
  });

  it("coming-soon state — dashed border + peach-50 bg + navy/25 value", () => {
    const { container } = render(
      <KpiCard
        label="월 매출"
        value="—"
    ...
    expect(card?.className).toContain("bg-hesya-peach-50/60");
    expect(container.querySelector(".text-hesya-navy-900\\/25")).toBeTruthy();
