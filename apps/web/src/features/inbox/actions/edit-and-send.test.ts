...
vi.mock("@/shared/lib/dal/messages", () => ({
  ...
  markDraftEdited: vi.fn(async () => undefined),
...
const { sendOutboundMock } = vi.hoisted(() => ({
  sendOutboundMock: vi.fn(async () => ({ externalMessageId: "out_e" })),
}));
...
import { editAndSend } from "./edit-and-send";
...
describe("editAndSend", () => {
  beforeEach(() => {
    ...
    sendOutboundMock.mockResolvedValue({ externalMessageId: "out_e" });
    setSession();
  });

  it("newText 비어 있음 → validation, 다른 호출 0회", async () => {
    const r = await editAndSend({ messageId: MSG_ID, newText: "" });
    expect(r).toEqual({ ok: false, error: "validation" });
    expect(findMessageById).not.toHaveBeenCalled();
  });

  it("다른 매장 메시지 → forbidden", async () => {
    vi.mocked(findMessageById).mockResolvedValue(
      mockMessage({ storeId: OTHER_STORE }) as never,
    );
    const r = await editAndSend({ messageId: MSG_ID, newText: "수정본" });
    expect(r).toEqual({ ok: false, error: "forbidden" });
    expect(markDraftEdited).not.toHaveBeenCalled();
  });
  ...
  it("happy path → markDraftEdited + IG send 시 newText 발송 + markMessageSent + ok", async () => {
    ...
    const r = await editAndSend({ messageId: MSG_ID, newText: "수정된 답변" });
    expect(r).toEqual({ ok: true });
    expect(markDraftEdited).toHaveBeenCalledWith(expect.anything(), {
      messageId: MSG_ID,
      newText: "수정된 답변",
    });
    expect(sendOutboundMock).toHaveBeenCalledWith(
      { externalRecipientId: "igsid_recipient", text: "수정된 답변" },
