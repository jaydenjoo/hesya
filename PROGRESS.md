...
### 🔭 추가 요구사항 (본 세션 후반 발생) — 외부인 데모 계정

Jayden이 "실제 시스템에 다른 사람들이 테스트할 수 있게 데모 계정"도 원함. ngrok 시연(개인용)을 넘어 **외부인이 데모 URL 직접 접근**해 사장 입장 시뮬할 환경. **차원이 다른 별도 인프라**.

**4 옵션 검증 결과** (전체 매트릭스: L-081 learnings.md):

| 옵션                                           | 격리      | 비용    | 🔴 RED 적합                          |
| ---------------------------------------------- | --------- | ------- | ------------------------------------ |
| A. prod에 데모 매장 + 공유 Google 계정         | ❌        | $0      | ❌ 즉시 탈락 (prod 데이터 사고 위험) |
| B. prod에 demo 모드 + RLS 격리                 | 🟡        | $0      | ❌ RLS 실수 = prod 노출              |
| **C. demo.hesya.com + 신규 Supabase 프로젝트** | ✅ 물리적 | ~$25/월 | ✅ **권장 (Phase 2)**                |
| D. Vercel Preview 데모 시드                    | ✅ 임시   | $0      | ⚠️ URL 휘발성, 공유 한계             |

**단계적 합의**:

- **Phase 1 (현재 plan)**: 로컬 시드 + ngrok — 본인 + 휴대폰 시연
- **Phase 2 (베타 매장 매칭 직전)**: 옵션 C `demo.hesya.com` + 데모 계정 1~3개 (매직링크) + 매일 자정 cron 시드 리셋
  - ⚠️ Supabase 신규 프로젝트 = **Jayden 사전 승인 필수** (외부 리소스 생성 정책)
  - 별도 PRD/Plan 작성 후 진행, 본 세션 scope X
