...
### [2026-05-07] L-081 — 데모 환경 4-옵션 매트릭스 (🔴 RED 프로젝트는 물리적 격리만 정답)

**상황 / 설계 결정**: Phase 1-β 코드 머지 후 Jayden 요청 _"실제 시스템에 다른 사람들이 테스트할 수 있게 데모 계정 만들어 사용 가능하게"_. 베타 매장 미매칭 단계 → 외부인이 사장 입장으로 클릭 시뮬할 환경 필요. ngrok과 별개 인프라.

**옵션 매트릭스 (검증 결과)**:

| 옵션                                                                   | 격리도        | 비용    | 보안 적합 (🔴 RED)                                                                                        |
| ---------------------------------------------------------------------- | ------------- | ------- | --------------------------------------------------------------------------------------------------------- |
| A. prod에 데모 매장 + Google 계정 비밀번호 공유                        | ❌ DB 같이 씀 | $0      | ❌ **즉시 탈락** — 데모 사용자가 prod 매장/고객 데이터 접근 가능, 사고 또는 의도적 유출 위험              |
| B. prod에 "demo 모드" 토글 + RLS namespace 격리                        | 🟡 SQL 격리   | $0      | ❌ RLS 1줄 실수 = prod 데이터 즉시 노출. 🔴 등급에서 single-DB 격리는 부족                                |
| **C. `demo.hesya.com` + 신규 Supabase 프로젝트 + 별도 Vercel project** | ✅ 물리적     | ~$25/월 | ✅ **유일한 정공법**. 데모 사고가 prod에 영향 0. Supabase 신규 = 외부 리소스 생성 = Jayden 사전 승인 필수 |
| D. Vercel Preview에 데모 시드                                          | ✅ 임시       | $0      | ⚠️ URL 휘발성 (PR 닫으면 사라짐), 공유 데모 계정으로 부적합. 한 번 보여주는 용도만                        |

**원인 / 왜 보수적**:

- 🔴 등급은 외부인 접근 = 잠재적 공격 벡터로 가정해야 함
- prod DB와 같은 물리 인스턴스에 데모 데이터 두면 → SQL injection / RLS 우회 / 권한 오설정 시 prod 데이터 노출
- 🔴 등급에서 "데이터 분리"는 schema/RLS만으로 부족 — DB 인스턴스 자체 분리가 baseline

**해결 / 단계적 합의**:

- **Phase 1**: 로컬 시드 + ngrok (본 세션 Plan v1) — 본인 + 휴대폰 시연용. 외부 공개 X
- **Phase 2**: 옵션 C — 별도 PRD/Plan 작성 후 진행. 베타 매장 매칭 직전 시점

**규칙** ⭐:

1. **🔴 RED 프로젝트의 데모 환경은 물리적 격리(별도 DB 인스턴스)가 baseline**. RLS namespace 격리만으로는 부족.
2. **prod 자격증명(IG 토큰, Auth secret 등)은 데모 환경에 절대 복사 금지**. 데모는 mock 토큰만 + 실제 외부 API 호출 차단.
3. **데모 계정 인증은 Google OAuth 공유 금지**. 매직링크 또는 데모 전용 토큰 발급 흐름. 비밀번호 공유는 보안 사고의 시작.
4. **데모 데이터는 시간 격리** — 매일 자정 cron으로 자동 시드 리셋. 데모 사용자가 입력한 잔여 데이터가 누적되지 않게.
5. **데모 환경 도입 = Supabase/Vercel/도메인 등 외부 리소스 신규 생성 = Jayden 사전 승인 의무**. CLI `--yes`로 무시 금지 (memory: 외부 리소스 생성 정책).
6. **단계적 도입**: 베타 매장 미매칭 단계에선 옵션 C 비용($25/월~) 정당화 약함 → ngrok으로 충분. **베타 매장 매칭 직전**이 옵션 C 트리거.

**확인 방법**:

- 인간 리뷰: Phase 2 plan에 위 6 규칙 모두 반영 여부 체크
- 자동: 데모 환경 IG 토큰이 mock 문자열인지 startup assert (예: `if (env.IG_API_KEY.startsWith("real_")) throw`)
...
### [2026-05-09] L-085 — 시연 prerequisite 3-layer (마이그 / seed / dev:demo DATABASE_URL) 격리 검증 의무. E12-4 분쟁처리 시연이 같은 영역 PR 4개 누적(#92→#93→#94→#95) 후 통과 — L-082 회고 trigger 발동.

**증상**: PR #93 (E12-4 UI) 머지 후 `pnpm dev:demo` → `/ko/store/disputes` 진입 시 `/sign-in` redirect. admin 페이지는 진입 가능하지만 분쟁 0건. Playwright 자동 시연 차단.

**원인 (3-layer 누적)**:

1. **Layer 1 (마이그)**: 마이그 0023_disputes.sql을 prod Supabase에만 적용 → 로컬 DB(HESYA_TEST_DATABASE_URL)에 `disputes` 테이블 없음. `pnpm seed:demo` 실행 시 `relation "disputes" does not exist`. → `pnpm --filter @hesya/database db:apply`로 해결.

2. **Layer 2 (seed)**: `seed-beta-demo.ts`가 `apps/web/src/shared/lib/dal/disputes.ts`의 `createDispute`를 직접 import. 해당 모듈은 `import "server-only"` 사용 → tsx에서 throw. fixture(`apps/web/e2e/fixtures/db.ts`) 헤더 주석이 명시한 server-only 우회 패턴(inline 재구현)을 위반. → PR #94 fix: `e2e/fixtures/db.ts`에 `seedDispute` helper 추가, `resetDb`에 `disputes` 테이블 추가.

3. **Layer 3 (dev:demo DATABASE_URL)**: `pnpm dev:demo`의 next dev 서버는 `.env.local`의 `DATABASE_URL`(prod Supabase) 그대로 사용. seed는 별도 `HESYA_TEST_DATABASE_URL`(로컬 DB)에 들어가지만 dev 서버는 prod DB 조회 → `store_owners`에 DEMO_USER_ID 매칭 row 없음 → ForbiddenError → `/sign-in` redirect. admin 페이지(`requireAdminEmail` bypass)는 store_owners 조회 안 해서 진입 자체는 OK였지만 분쟁/매장도 prod DB라 0건. → PR #95 fix: `dev-demo.sh`가 next dev 기동 직전에 dotenv로 `.env.local` 파싱 → `DATABASE_URL="$HESYA_TEST_DATABASE_URL"` inline override.

**프로세스 회고**:

- PR #93 plan v1 단계에서 인벤토리 절차는 충실히 수행 (`grep dispute` 결과 0건 확인, 작업 영역 ls, 마이그 파일 확인). 하지만 **시연 prerequisite 3-layer 매트릭스 미점검**.
- 글로벌 룰 "같은 영역 PR 3개+ 누적 시 회고 trigger" — 본 세션 4개째 (#92 backend, #93 UI, #94 fix seed, #95 fix dev:demo). 명시 trigger 발동.
- L-082가 "PROGRESS 자기평가는 e2e 시연 기준"이라고 했으나, **시연 prerequisite의 환경 격리 자체가 암묵적 가정**이었음. 명시화 필요.

**해결**: 4개 PR 통과 후 `pnpm dev:demo` 재시작 → Playwright e2e 8단계 자동 시연 통과.

**규칙** ⭐:

1. **시연 prerequisite 3-layer 매트릭스를 plan v1 인벤토리 섹션에 의무 첨부** — 새 테이블/feature 동반 시:

   | Layer       | 검증 명령                                                      | 통과 기준                                                |
   | ----------- | -------------------------------------------------------------- | -------------------------------------------------------- |
   | 마이그      | `ls packages/database/migrations/*<feature>*.sql`              | 해당 마이그 파일 존재 + prod **+ 로컬 둘 다** 적용       |
   | seed        | `grep "<feature>" apps/web/scripts/seed-beta-demo.ts`          | seed 호출 + e2e fixture에 helper 존재 (server-only 우회) |
   | dev 서버 DB | `dev-demo.sh`의 `DATABASE_URL` source 명시 / `.env.local` 파싱 | seed와 dev 서버가 같은 DB 가리킴                         |

2. **fixture(`apps/web/e2e/fixtures/db.ts`)는 server-only 우회 패턴 — 새 테이블 추가 시 fixture에 inline `seedX` helper + `resetDb`에 테이블 delete 추가 의무**. seed-beta-demo.ts에서 DAL 직접 import 절대 금지.

3. **`pnpm --filter @hesya/database db:apply`는 시연 setup 절차의 의무 첫 단계**. 마이그 추가 PR이 prod 적용했어도 로컬 DB에는 자동 적용되지 않음.

4. **dev-demo.sh의 DATABASE_URL override는 영구 패턴**. `.env.local`의 DATABASE_URL이 prod이든 local이든 dev:demo는 무조건 `HESYA_TEST_DATABASE_URL` 사용. 시연 격리 의도 명세.

5. **같은 영역 PR 3개+ 누적 시 즉시 plan 인벤토리 부실 회고** — 본 L-085가 그 회고. plan은 인벤토리를 했지만 검증 매트릭스가 부족했음.

**확인 방법**:

- 자동 (후보): plan v1 template에 "시연 prerequisite 3-layer 체크리스트" 섹션 추가
- 자동 (후보): 새 마이그 파일 추가 PR에 dev:demo 재시작 + Playwright smoke test 자동 트리거
- 인간 리뷰: 새 feature plan에 마이그/seed/dev:demo DATABASE_URL 3-layer 명시 검증 표기
- 메타: 향후 3개월 새 feature plan에서 시연 차단 fix PR 0건이면 영구 패턴 정착

**비유**: 무대 공연 리허설 — 무대 세트(마이그)를 본극장에만 설치하고 리허설장(로컬)에 안 옮기면 배우(dev:demo)가 무대 세트 없는 곳에서 연기하게 됨. 또는 배우의 대본(DATABASE_URL)이 본극장 버전이라 리허설장 소품과 안 맞음. 격리된 무대 환경에서 e2e 리허설이 가능해야 진짜 공연 직전 검증 의미.

**연관**: L-078 (PRD-only planning 함정 — 인벤토리 의무 도입), L-079 (5-Layer 문서 구조), L-082 (PROGRESS 자기평가 e2e 시연 기준 + 같은 영역 PR 3개+ 회고 trigger), L-084 (새 env 도입 5-layer 정합성), 본 세션 PR #92 `b429245` + #93 `5ece42b` + #94 `766076e` + #95 squash + Playwright e2e 8단계 통과 (`apps/web/demo-step{1,3,8}-*.png`).

### [2026-05-10] L-086 — PR squash merge 후 fix 커밋 push는 main 미반영. 보안 review fix가 prod에 도달하지 못한 사고 + 새 PR로 재진입 영구 차단 패턴.

**증상**:

PR #99 (E12-8 API정책 RSS) squash merge 후 보안 review subagent가 CRITICAL 3건(timing-safe 비교 / link XSS / alertId 노출) 발견. fix 커밋 `79b0b9e`을 origin branch에 push했으나 PR이 이미 closed 상태라 main에 도달하지 않음. 이후 머지된 PR #100 prod 배포 시 N8N_WEBHOOK_SECRET 미등록 ZodError로 또 fail → 종합적으로 prod에 보안 fix가 한 번도 land되지 않음.

```bash
# Jayden 질문: "fix 수정완료한건가?"
$ git log origin/main --oneline | head -3
eb40da4 feat(api-policy-alerts): E12-8 ... (#99)   # squash = 8a90aae(초기) 1건만
ed2ae85 feat(ai-accuracy): ... (#98)
68cbce3 feat(payment-monitoring): ... (#97)

# fix 커밋은?
$ git log origin/feat/api-policy-alerts-e12-8 --oneline | head -3
79b0b9e fix(api-policy-alerts): security review 잔여 패치   ← main에 없음
8a90aae feat(api-policy-alerts): ...
```

**원인**:

1. **Squash merge 시점의 PR head SHA만 통합**: GitHub의 squash merge는 PR head SHA(머지 시점)의 누적 변경분 1개 커밋으로 통합. 이후 origin branch에 push된 커밋은 main에 자동 반영되지 않음.
2. **CI 그린 후 머지된 시점 vs fix push 시점의 race**: subagent 보안 review를 PR 머지 후 진행 → fix는 이미 closed된 PR origin branch에 push만 되고 끝. 자동 머지 라벨이 closed PR을 다시 열지 않음.
3. **휴먼 워크플로 가정 깨짐**: "PR 머지 = 모든 변경 반영"이라는 직관이 squash + fix push 시퀀스에서 깨짐. 특히 auto-merge 라벨로 무인 머지될 때 fix push 타이밍을 사람이 인지하기 어려움.

**해결**:

1. PR #100을 별도 브랜치 `fix/api-policy-alerts-security-review`로 새로 생성 + auto-merge 라벨 → CI 그린 → 자동 머지 (`4387501`).
2. 같은 fix 8개 파일(150+/34-)을 commit + PR title에 "PR #99 누락분" 명시.

**규칙** ⭐:

1. **보안 review subagent는 PR 머지 BEFORE에 의무 실행** — 머지 직전 + auto-merge 라벨 부여 직전. 머지 AFTER fix는 무조건 새 PR.
2. **PR 머지 직전 main HEAD 검증**: auto-merge 라벨 부여 전 `git fetch origin main && git diff origin/main..HEAD` 또는 GitHub PR "Files changed" 탭에서 fix 포함 여부 확인.
3. **PR closed 후 origin branch에 push 금지** — push는 새 PR에서만. 헷갈림 차단.
4. **squash merge 정책 인지**: 본 프로젝트는 squash merge가 기본. PR head SHA 시점 변경분만 통합되는 점 plan/checklist에 명시.
5. **자동 머지 라벨 워크플로 점검**: `auto-merge` 라벨이 closed PR을 다시 열지 않도록 보장 (현재 yml 동작 확인 필요).

**확인 방법**:

- 자동 (후보): GitHub Actions에서 PR "review approved" 후에만 auto-merge 라벨 적용 가능하도록 워크플로 강화
- 자동 (후보): security-reviewer subagent를 plan v1 단계 + PR 머지 직전 단계 둘 다 실행 (양쪽 게이트)
- 인간 리뷰: PR 머지 직전 GitHub "Files changed" 탭 5초 확인이 표준 절차로 정착
- 메타: 향후 3개월 squash merge 후 fix 누락 사고 0건이면 영구 패턴 정착

**비유**: 출판 마감 — 원고를 인쇄소에 보내고 나서 발견한 오타는 인쇄된 책에 반영되지 않음. 다음 판본(새 PR)에서나 가능. 그러니 인쇄소에 보내기 직전이 마지막 교정 기회. 보안 review는 "인쇄소 보내기 직전 최종 교정" 단계여야 함, 인쇄 후 정오표 같은 사후 fix 시퀀스가 아님.

**연관**: L-082 (PROGRESS 자기평가 e2e 시연 기준 — main HEAD가 prod 코드라는 전제 강화), L-085 (시연 prerequisite 격리 검증), 본 세션 PR #99 `eb40da4` (squash) + `79b0b9e` (orphan) + PR #100 `4387501` (재진입 머지).

### [2026-05-10] L-087 — 새 env 도입 PR은 Vercel Production+Preview+Development 3환경 등록까지 6-layer 정합성. L-084의 5-layer는 Vercel 등록 layer 누락.

**증상**:

PR #99 (E12-8) 머지 후 Vercel Production 자동 배포가 ZodError로 fail:

```
Error [ZodError]: N8N_WEBHOOK_SECRET expected string, received undefined
  at /api/cron/revalidate-stores route build
```

PR #99 시점에 `apps/web/src/shared/config/env.ts`에 `N8N_WEBHOOK_SECRET: z.string().min(32)` 추가했으나 Vercel Dashboard 환경변수에는 등록 안 됨. PR #100 prod 배포도 같은 원인으로 fail. Jayden이 Vercel Dashboard에서 환경변수 등록 + 재배포로 해결까지 약 1h 소요.

**원인**:

1. **L-084 5-layer 정의가 Vercel 등록 누락**: L-084는 Zod schema / .env.local / turbo.json / CI workflow 4-layer만 명시 ("5-layer"라고 적혀 있지만 본문 4개). Vercel Dashboard 환경변수 등록은 별도 layer.
2. **`turbo.json`의 `passThroughEnv`는 "있을 때 통과"만 보장**: 변수 자체가 Vercel에 없으면 build 시점 `process.env.X = undefined` → zod parse 실패.
3. **build 시점 page data 수집의 영향 범위**: `/api/cron/revalidate-stores` 같은 cron route가 env import 체인을 통해 모든 env 변수 검증을 트리거. 새 env 1개라도 빠지면 무관해 보이는 route build가 깨짐.
4. **Preview 빌드는 통과한 이유**: PR Preview는 PR head SHA(`8a90aae` 초기 커밋, env.ts 미수정 시점)로 빌드되어 통과. fix 커밋(env.ts 수정 포함)의 Preview 빌드는 검증되지 않은 상태로 PR 머지됨 → prod에서 첫 발견.

**해결**:

1. `vercel env ls production`으로 누락 변수 확인 (`N8N_WEBHOOK_SECRET` 1개).
2. Vercel Dashboard에서 Production + Preview + Development 3환경 모두에 등록 (값은 `openssl rand -base64 32` 32바이트 임시값, 베타·prod 출시 직전 rotation 예정).
3. Deployments 탭에서 최신 main `4387501` Redeploy → success (08:10:39Z).

**규칙** ⭐:

1. **새 env 도입 PR 6-layer 정합성 의무** (L-084 4-layer 확장):

   | #   | Layer                  | 검증 명령                                                             | 통과 기준                            |
   | --- | ---------------------- | --------------------------------------------------------------------- | ------------------------------------ |
   | 1   | Zod schema             | `grep "<NAME>" apps/web/src/shared/config/env.ts`                     | 정의 존재 + 적절한 검증 (min/format) |
   | 2   | .env.local (개발)      | `grep -c "^<NAME>=" apps/web/.env.local`                              | 1                                    |
   | 3   | **Vercel Production**  | `vercel env ls production \| grep <NAME>`                             | 1 (Encrypted)                        |
   | 4   | **Vercel Preview**     | `vercel env ls preview \| grep <NAME>`                                | 1                                    |
   | 5   | **Vercel Development** | `vercel env ls development \| grep <NAME>`                            | 1                                    |
   | 6   | turbo.json             | `grep "<NAME>" turbo.json` (passThroughEnv 또는 globalPassThroughEnv) | 1                                    |
   | 7   | CI workflow            | `grep "<NAME>" .github/workflows/ci.yml`                              | 1 (dummy 또는 실제 secret)           |

   (실제로는 7-layer지만 "6-layer 정합성"은 Vercel 3환경을 1 layer로 묶은 표현. plan에는 7-layer 체크리스트로 적시.)

2. **Vercel 등록은 Jayden 수동 작업 (보안 🔴 RED 정책)** — AI는 등록 시도 금지, 안내만. plan v1 마무리 단계에 "Jayden Vercel Dashboard 등록 가이드" 섹션 의무 첨부.

3. **PR 머지 직후 Production 배포 모니터링** 의무 — `gh api repos/:owner/:repo/deployments?environment=Production` 조회 5분 후. 실패 시 즉시 진단 (env 누락이 가장 흔한 원인).

4. **임시 secret 값 사용 정책 명시** — 외부 서비스 미배포 단계의 secret은 32바이트 random으로 임시 사용 OK. 단 베타·prod 출시 직전 rotation 의무 (Jayden 글로벌 메모리: token rotation 정책).

5. **`/api/cron/revalidate-stores` 같은 env-heavy route 영향 인지**: 새 env 추가는 무관한 route의 build를 깨뜨릴 수 있음. build 단계에서 env parse 발생 시점 인지.

**확인 방법**:

- 자동 (후보): `scripts/verify-env-coverage.ts` — env.ts와 Vercel(`vercel env ls`) + .env.local + turbo.json + ci.yml 5개 source 비교 후 누락 출력
- 자동 (후보): GitHub Actions PR check에 새 env 변수 감지 시 "Vercel 등록 확인" 라벨 자동 부착
- 인간 리뷰: env.ts 변경 PR은 마지막 단계에 6-layer 체크리스트 (Markdown) PR description에 부착
- 메타: 향후 3개월 새 env PR Production 배포 fail 0건이면 영구 패턴 정착
