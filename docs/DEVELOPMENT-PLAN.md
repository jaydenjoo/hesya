# Visit K Phase 1 — 개발 계획서 (DEVELOPMENT-PLAN v1.2 FINAL)
...
| # | Task | 시간 |
|---|---|---|
| SS-1 | Supabase 별도 프로젝트 `visit-k-staging` 생성 | 2h |
...
git checkout main
git worktree add ../visit-k-inbox     -b feat/epic1-inbox
git worktree add ../visit-k-booking   -b feat/epic3-booking
git worktree add ../visit-k-kyc       -b feat/epic9-kyc
git worktree add ../visit-k-dashboard -b feat/epic4-dashboard
git worktree add ../visit-k-admin     -b feat/epic12-admin   # ⭐ v1.1 신규
git worktree add ../visit-k-payment   -b feat/epic2-payment  # Day 16~ Lead 직접
```
...
if [ $? != 0 ]; then
  ...
  # Window 0: Lead
  tmux send-keys -t $SESSION:0 "cd ~/visit-k && claude --model opus" Enter
  
  # Window 1: Inbox Worker
  tmux new-window -t $SESSION:1 -n "inbox"
  tmux send-keys -t $SESSION:1 "cd ~/visit-k-inbox && claude --model sonnet --agent worker-inbox" Enter
  
  # Window 2: Booking Worker
  tmux new-window -t $SESSION:2 -n "booking"
  tmux send-keys -t $SESSION:2 "cd ~/visit-k-booking && claude --model sonnet --agent worker-booking" Enter
  
  # Window 3: KYC Worker (🔴 보안)
  tmux new-window -t $SESSION:3 -n "kyc"
  tmux send-keys -t $SESSION:3 "cd ~/visit-k-kyc && claude --model sonnet --agent worker-kyc" Enter
  
  # Window 4: Dashboard Worker (Day 16~)
  tmux new-window -t $SESSION:4 -n "dashboard"
  tmux send-keys -t $SESSION:4 "cd ~/visit-k-dashboard && claude --model sonnet --agent worker-dashboard" Enter
  
  # Window 5: Admin Worker (Day 15~) ⭐ v1.1 신규
  tmux new-window -t $SESSION:5 -n "admin"
  tmux send-keys -t $SESSION:5 "cd ~/visit-k-admin && claude --model sonnet --agent worker-admin" Enter
  
  # Window 6: Reviewer
  tmux new-window -t $SESSION:6 -n "reviewer"
  tmux send-keys -t $SESSION:6 "cd ~/visit-k && claude --model opus --agent security-reviewer" Enter
  
  # Window 7: Monitor
  tmux new-window -t $SESSION:7 -n "monitor"
  tmux send-keys -t $SESSION:7 "cd ~/visit-k && pnpm supabase logs -f" Enter
fi
...
1. GitHub Private Repo 생성 (`visit-k`)
