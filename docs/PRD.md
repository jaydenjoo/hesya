...
> **문서 정보**
>
> - **버전**: v1.3 (Plan v4 2026-05-14 — 모듈 5 부가세 환급 삭제 / 사업자 필요 모듈 mock 명시)
> - **작성일**: 2026-04-29 (v1.2 기준) / 2026-05-14 (v1.3 갱신)
> - **이전 버전**: v1.0 → v1.1 → v1.2 → v1.3 변경 사항은 § Appendix B 참조
...
> **v1.3 주요 변경 (Plan v4)**:
>
> 1. **모듈 5 사후면세점 부가세 환급 자동화 완전 삭제** — 미용업의 조세특례제한법 107조 적용 불확실 + 1인 운영 단계 ROI 약함.
> 2. **사업자 필요 모듈 mock 명시** — 결제 6채널 (Stripe/Alipay+/WeChat/LINE Pay/PayPay/UnionPay) / 메신저 4채널 (WhatsApp/Kakao/LINE/Facebook) / 슬롯 동기화 (네이버·카카오) / 후기 수집 (Google·Naver·샤오홍슈·Insta) — `*_MOCK_MODE=true` env 토글로 mock UI 100% 구현. 사업자 등록 후 swap.
> 3. **나머지 디자인-구현 갭 100% 폐쇄** — AI Photo Analysis / Chat 대면 통역 / Owner Analytics 페이지 진짜 개발.
>
> - **부속 문서**:
...
| ~~R-C6~~ | ~~부가세 자동 환급 (사후면세점)~~ — **Plan v4 제거** (2026-05-14) | —        |
...
### ~~모듈 5. 사후면세점 부가세 환급 자동화~~ — Plan v4 제거 (2026-05-14)

> Plan v4에서 본 모듈 완전 삭제. 미용업의 조세특례제한법 107조 적용 가능성 불확실 + 변호사 자문 의존 + 1인 운영 단계의 ROI 약함. 후속 Phase에서 재검토 가능.
...
| **워크플로우**          | n8n                                         | 메시지 발송·후기 크롤링·B2B 영업 자동화 (Elest.io 셀프호스팅)                    |
...
> ⚠️ **본 표는 고정 인프라 비용만 포함합니다 (AI 변동비 별도).**
> AI 모델 호출 비용(Claude Sonnet/Opus, OpenAI Whisper 등)은 사용량 기반 AI 변동비로
...
| 토큰               | 색상      | 용도                                                                     |
| ------------------ | --------- | ------------------------------------------------------------------------ |
| `--kverified-gold` | `#D4AF37` | KYC 통과 매장 골드 뱃지 (`stores.verification_status = 'auto_approved'`) |
...
- 글로벌텍스프리와 **사후면세점 연동 MOU** (2025.02) ⚠️ Plan v4에서 모듈 5 제거 → 본 영역 침범 무관해짐
...
### Epic 7~8. Phase 1.5 (Day 60~120)

- ~~Epic 6: 사후면세점 환급 자동화~~ — Plan v4 제거 (모듈 5 삭제)
