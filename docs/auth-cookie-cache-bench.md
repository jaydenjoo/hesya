...
- **Warm avg (iter 1~5)**: 같은 페이지 재방문 — cookie cache HIT, DB SELECT 없음

5개 인증 owner 페이지 × 6회씩 = 30 nav 측정. waitUntil: "commit"으로 TTFB 근사 (첫 HTML byte 도착).
...
- **N=1 (단일 측정 세션)** — Vercel cold start, 네트워크 변동 등으로 실측 분산 큼. 의미 있는 정량은 통계 처리 필요 (n≥10).
- **prod 단일 매장 데이터** — demo 계정 1개라 multi-store contention 미반영.
- **5분 TTL 경계 미테스트** — 6 iter는 보통 30~60s 안에 완료. 5분 후 cache expiry 동작은 별도 테스트 필요.
...
1. **N≥10 회 반복** — median + p95 비교 (avg는 outlier에 약함)
